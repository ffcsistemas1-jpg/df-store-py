-- Hace que la Dirección deje de ser obligatoria para "delivery" (Asunción y
-- Central) cuando el cliente ya compartió su ubicación por GPS o un link de
-- Maps. Para "interior" (envío por transportadora) la dirección se sigue
-- pidiendo, porque ahí sí es imprescindible.
--
-- Primero se eliminan versiones anteriores de create_order (por si quedó
-- alguna con una firma distinta de un patch previo) para evitar que Postgres
-- tenga dos funciones con el mismo nombre y falle al llamarla.
drop function if exists public.create_order(jsonb,jsonb,text,text,uuid,text,jsonb,boolean);
...
begin
 ...
 if p_delivery_type='interior' and coalesce(trim(p_customer->>'address'),'')='' then raise exception 'La dirección es obligatoria para envíos al interior'; end if;
