...
create table if not exists public.shipping_companies(
 id uuid primary key default gen_random_uuid(), name text unique not null, phone text, notes text, active boolean default true
);
...
insert into public.store_settings(id, whatsapp) values (1, null)
