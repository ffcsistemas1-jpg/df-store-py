...
export default function Checkout(){
 ...
 const canContinueStep1=useMemo(()=>{if(!form.full_name.trim()||!form.whatsapp.trim())return false;if(form.delivery_type==="delivery"&&!form.city)return false;if(form.delivery_type==="interior"&&(!form.department||!form.city||!form.address.trim()))return false;if(form.delivery_type==="delivery"&&!zoneResolved)return false;if(form.delivery_type==="interior"&&!form.shipping_company_id)return false;return true},[form.full_name,form.whatsapp,form.delivery_type,form.department,form.city,form.address,zoneResolved,form.shipping_company_id]);
 const validateStep1=()=>{if(!form.full_name.trim()||!form.whatsapp.trim()){setMsg("Completá nombre y WhatsApp.");return false}if(form.delivery_type==="delivery"&&!form.city){setMsg("Completá la ciudad para el delivery.");return false}if(form.delivery_type==="interior"&&(!form.department||!form.city||!form.address.trim())){setMsg("Completá los datos de entrega obligatorios.");return false}if(form.delivery_type==="delivery"&&!zoneResolved){setMsg("Seleccioná una zona con tarifa de delivery disponible.");return false}if(form.delivery_type==="interior"&&!form.shipping_company_id){setMsg("Seleccioná una transportadora.");return false}setMsg("");return true};
