...
export default function Checkout(){
 ...
 const [locationStatus,setLocationStatus]=useState(""); const [showNote,setShowNote]=useState(false); const sessionRef=useRef(""); const draftReady=useRef(false); const [restoredDraft,setRestoredDraft]=useState(false); const initiateTracked=useRef(false);
 ...
 useEffect(()=>{if(!draftReady.current)return;if(!form.full_name.trim()&&!form.whatsapp.trim()&&!form.address.trim())return;const timer=window.setTimeout(()=>{const session=sessionRef.current;if(!session)return;const s=createClient();s.rpc("save_checkout_draft",{p_session:session,p_draft:{full_name:form.full_name||null,whatsapp:form.whatsapp||null,email:form.email||null,department:form.department||null,city:form.city||null,neighborhood:form.neighborhood||null,address:form.address||null,delivery_type:form.delivery_type||null,payment_method:form.payment_method||null,shipping_company_id:form.shipping_company_id||null,preferred_time:form.preferred_time||null,invoice_requested:form.invoice_requested,maps_url:form.maps_url||null,note:form.note||null}}).then(({error}:any)=>{if(error)console.error("No se pudo guardar el borrador",error)})},700);return()=>window.clearTimeout(timer)},[form]);
 ...
 const canContinueStep1=useMemo(()=>{if(!form.full_name.trim()||!form.whatsapp.trim())return false;if(!isValidPyWhatsapp(form.whatsapp))return false;if(form.delivery_type==="delivery"&&!form.city)return false;if(form.delivery_type==="interior"&&(!form.department||!form.city||!form.address.trim()))return false;if(form.delivery_type==="delivery"&&!zoneResolved)return false;if(form.delivery_type==="interior"&&!form.shipping_company_id)return false;return true},[form.full_name,form.whatsapp,form.delivery_type,form.department,form.city,form.address,zoneResolved,form.shipping_company_id]);
 const validateStep1=()=>{if(!form.full_name.trim()||!form.whatsapp.trim()){setMsg("Completá nombre y WhatsApp.");return false}if(!isValidPyWhatsapp(form.whatsapp)){setMsg("Revisá tu número de WhatsApp: parece incompleto o mal escrito.");return false}if(form.delivery_type==="delivery"&&!form.city){setMsg("Completá la ciudad para el delivery.");return false}if(form.delivery_type==="interior"&&(!form.department||!form.city||!form.address.trim())){setMsg("Completá los datos de entrega obligatorios.");return false}if(form.delivery_type==="delivery"&&!zoneResolved){setMsg("Seleccioná una zona con tarifa de delivery disponible.");return false}if(form.delivery_type==="interior"&&!form.shipping_company_id){setMsg("Seleccioná una transportadora.");return false}setMsg("");return true};
 ...
 async function submit(){
  if(busy||order) return; // evita doble envío / doble Purchase para el mismo pedido
  setBusy(true);setMsg("");
  try{
   ...
   const {data,error}=await s.rpc("create_order",{
    p_customer:{full_name:form.full_name,whatsapp:form.whatsapp,email:form.email||null,department:form.department,city:form.city,neighborhood:form.neighborhood,address:form.address,preferred_time:form.preferred_time,invoice_requested:form.invoice_requested,maps_url:form.maps_url||null,note:form.note||null},
    p_items:items.map(i=>({id:i.id,quantity:i.quantity})),
    p_delivery_type:form.delivery_type,p_payment_method:form.payment_method,p_shipping_company_id:form.shipping_company_id||null,p_payment_reference:paymentReference.trim()||null,
 ...
 <div className="checkout-fields"><label>Nombre y apellido*<input required value={form.full_name} onChange={e=>set("full_name",e.target.value)} /></label><label>Teléfono / WhatsApp*<input required value={form.whatsapp} onChange={e=>set("whatsapp",e.target.value)} placeholder="09XX XXX XXX" />{form.whatsapp.trim()&&!isValidPyWhatsapp(form.whatsapp)&&<small className="field-error">⚠️ Revisá el número, parece incompleto (ej: 0982 625 643)</small>}</label><label>Email<input type="email" value={form.email} onChange={e=>set("email",e.target.value)} placeholder="Opcional" /></label>
 {form.delivery_type==="delivery"&&<div className="location-box"><h3>📍 Compartí tu ubicación real</h3><p>Así llegamos directo a tu casa, sin tener que adivinar el barrio exacto.</p><button type="button" className="location-btn" onClick={location}>📍 Usar mi ubicación actual</button>{locationStatus&&<small>{locationStatus}</small>}<div className="location-or"><span></span><b>O</b><span></span></div><p><b>¿Preferís pegar un link?</b><br/><span>Podés pegar un enlace de Google Maps.</span></p><button type="button" className="location-btn secondary" onClick={pasteMaps}>📍 Pegar enlace de Google Maps</button>{form.maps_url&&<small>✓ Enlace de ubicación guardado.</small>}</div>}
 {form.delivery_type==="delivery"&&<><label>Ciudad / Distrito*<select required value={form.city} onChange={e=>setDeliveryCity(e.target.value)}><option value="">Seleccioná tu ciudad</option>{deliveryCities.map(c=>{const f=deliveryCityFee(c);return <option key={c} value={c}>{c}{f!==null?` — ${Number(f).toLocaleString("es-PY")}`:" — Sin tarifa"}</option>})}</select></label><label>Dirección <span className="field-optional">(opcional si ya compartiste tu ubicación)</span><input value={form.address} onChange={e=>set("address",e.target.value)} placeholder="Calle, número y referencia" /></label><label>Barrio / localidad <span className="field-optional">(opcional si ya compartiste tu ubicación)</span>{form.city?<select value={form.neighborhood} onChange={e=>set("neighborhood",e.target.value)} disabled={!neighborhoodsForCity.length}><option value="">{neighborhoodsForCity.length?"Seleccioná tu barrio / localidad":"Cargando barrios / localidades..."}</option>{neighborhoodsForCity.map(n=><option key={n} value={n}>{n}</option>)}</select>:<select disabled><option>Primero elegí tu ciudad</option></select>}</label></>}
 {form.delivery_type==="interior"&&<><label>Departamento*<select required value={form.department} onChange={e=>setDepartment(e.target.value)}><option value="">Seleccioná tu departamento</option>{departments.map(d=><option key={d} value={d}>{d}</option>)}</select></label><label>Ciudad / Distrito*<select required value={form.city} onChange={e=>setCity(e.target.value)} disabled={!form.department}><option value="">{form.department?"Seleccioná tu ciudad":"Primero elegí tu departamento"}</option>{citiesForDept.map(c=><option key={c} value={c}>{c}</option>)}</select></label><label>Dirección*<input required value={form.address} onChange={e=>set("address",e.target.value)} placeholder="Calle, número y referencia" /></label><label>Barrio / localidad <span className="field-optional">(opcional)</span>{form.city?<select value={form.neighborhood} onChange={e=>set("neighborhood",e.target.value)} disabled={!neighborhoodsForCity.length}><option value="">{neighborhoodsForCity.length?"Seleccioná tu barrio / localidad":"Cargando barrios / localidades..."}</option>{neighborhoodsForCity.map(n=><option key={n} value={n}>{n}</option>)}</select>:<select disabled><option>Primero elegí tu ciudad</option></select>}</label></>}
 ...
 <details className="checkout-more"><summary>+ Más opciones (horario, factura)</summary>
