...
export default function Checkout() {
  ...
  // Guarda el borrador en cuanto el cliente empieza a identificarse.
  // El teléfono es el dato mínimo y prioritario: no depende de que avance
  // de paso ni de que complete nombre/dirección.
  const saveCheckoutDraft = async (patch: Partial<FormState> = {}) => {
    const session = sessionRef.current || getCartSession();
    if (!session) return;
    sessionRef.current = session;
    const draft = {
      full_name: (patch.full_name ?? form.full_name) || null,
      whatsapp: (patch.whatsapp ?? form.whatsapp) || null,
      email: (patch.email ?? form.email) || null,
      department: (patch.department ?? form.department) || null,
      city: (patch.city ?? form.city) || null,
      neighborhood: (patch.neighborhood ?? form.neighborhood) || null,
      address: (patch.address ?? form.address) || null,
      delivery_type: (patch.delivery_type ?? form.delivery_type) || null,
      payment_method: (patch.payment_method ?? form.payment_method) || null,
      shipping_company_id: (patch.shipping_company_id ?? form.shipping_company_id) || null,
      shipping_company_other: (patch.shipping_company_other ?? form.shipping_company_other) || null,
      preferred_time: (patch.preferred_time ?? form.preferred_time) || null,
      invoice_requested: patch.invoice_requested ?? form.invoice_requested,
      ruc: (patch.ruc ?? form.ruc) || null,
      business_name: (patch.business_name ?? form.business_name) || null,
      maps_url: (patch.maps_url ?? form.maps_url) || null,
      note: (patch.note ?? form.note) || null,
    };
    const { error } = await createClient().rpc("save_checkout_draft", {
      p_session: session,
      p_draft: draft,
    });
    if (error) console.error("No se pudo guardar el borrador", error);
  };
  ...
  useEffect(() => {
    ...
    if (!form.full_name.trim() && !form.whatsapp.trim() && !form.email.trim() && !form.address.trim()) return;
  ...
  async function submit() {
    ...
    try {
      ...
      await sendCapiEvent({
        ...
        phone: normalizePyWhatsapp(form.whatsapp),
      });
      clear();
  ...
  if (order) return <section className="checkout-section"><div className="checkout-shell"><div className="checkout-brand"><b>DF</b> STORE PY</div><div className="checkout-head"><small>PEDIDO CONFIRMADO</small><h1>Pedido confirmado</h1><p>{order.delivery_type === "interior" ? "Recibimos tu pedido y tu comprobante de pago. Nuestro equipo verificará el pago antes de preparar el despacho." : "Gracias por tu compra. Guardá tu número de pedido."}</p></div><div className="checkout-panel panel"><h2>N.º de pedido: {order.id.slice(0, 8).toUpperCase()}</h2><div className="order-lines"><div><span>Subtotal</span><b>{money(order.subtotal)}</b></div><div><span>{order.delivery_type === "delivery" ? "Delivery" : "Transporte"}</span><b>{order.delivery_type === "delivery" ? money(order.delivery_fee) : "A confirmar"}</b></div><div className="checkout-total grand"><span>Total</span><strong>{money(order.total)}</strong></div></div><a className="btn checkout-whatsapp" href={`https://wa.me/${normalizePyWhatsapp(whatsapp)}`} target="_blank" rel="noreferrer">Contactar por WhatsApp</a></div></div></section>;
  ...
  return <section className="checkout-section"><div className="checkout-shell"><div className="checkout-brand"><b>DF</b> STORE PY</div><div className="checkout-head"><small>CHECKOUT · PASO {step} DE 3</small><h1>Finalizar compra</h1><p>Completá tus datos. Es rápido y seguro.</p>{restoredDraft && <div className="draft-notice">Retomamos los datos que ya habías completado.</div>}</div><div className="checkout-steps" aria-label="Progreso"><span className={step >= 1 ? "active" : ""}>1. Entrega</span><span className={step >= 2 ? "active" : ""}>2. Revisar pedido</span><span className={step >= 3 ? "active" : ""}>3. Pago</span></div>
    {step === 1 && <div className="checkout-panel panel"><h2>📍 ¿Dónde entregamos?</h2><div className="delivery-choice"><label className={form.delivery_type === "delivery" ? "choice active" : "choice"}><input type="radio" name="delivery" checked={form.delivery_type === "delivery"} onChange={() => setForm(x => ({ ...x, delivery_type: "delivery", payment_method: "Pago al recibir" }))} /><span><b>Asunción y Central</b><small>Delivery y pago al recibir</small></span></label><label className={form.delivery_type === "interior" ? "choice active" : "choice"}><input type="radio" name="delivery" checked={form.delivery_type === "interior"} onChange={() => setForm(x => ({ ...x, delivery_type: "interior", payment_method: "Transferencia" }))} /><span><b>Interior del país</b><small>Envío por transportadora · pago anticipado</small></span></label></div><div className="delivery-info"><b>🚚 {form.delivery_type === "delivery" ? "Delivery local" : "Envío al interior"}</b><span>{form.delivery_type === "delivery" ? "Pagás al recibir en zonas habilitadas." : "El producto se paga anticipadamente por transferencia bancaria o Giro Tigo. El costo del transporte se abona directamente a la transportadora."}</span></div><div className="checkout-fields"><label>Nombre y apellido*<input required value={form.full_name} onChange={e => set("full_name", e.target.value)} placeholder="Tu nombre completo" /></label><label>WhatsApp*<input required value={form.whatsapp} onChange={e => { const value = e.target.value; set("whatsapp", value); void saveCheckoutDraft({ whatsapp: value }); }} onBlur={() => { void saveCheckoutDraft(); }} placeholder="09xx xxx xxx" /></label><label>Email <span className="field-optional">(opcional)</span><input type="email" value={form.email} onChange={e => set("email", e.target.value)} placeholder="tu@email.com" /></label>{form.delivery_type === "delivery" && <><div className="location-box"><div><b>📍 Ubicación para facilitar la entrega</b><span>{form.maps_url ? "Ya compartiste tu ubicación." : "Podés compartir tu ubicación exacta o pegar un enlace de Google Maps."}</span>{locationStatus && <small>{locationStatus}</small>}</div><div className="location-actions"><button type="button" className="location-btn" onClick={location}>Usar mi ubicación</button><button type="button" className="location-btn secondary" onClick={pasteMaps}>Pegar enlace</button></div></div><label>Ciudad / Distrito*<select required value={form.city} onChange={e => setDeliveryCity(e.target.value)}><option value="">Seleccioná tu ciudad</option>{deliveryCities.map(c => { const f = deliveryCityFee(c); return <option key={c} value={c}>{c}{f !== null ? ` — ${Number(f).toLocaleString("es-PY")}` : " — Sin tarifa"}</option>; })}</select></label><label>Dirección <span className="field-optional">(opcional si ya compartiste tu ubicación)</span><input value={form.address} onChange={e => set("address", e.target.value)} placeholder="Calle, número y referencia" /></label></>}
      ...
      <label className="invoice-check standalone-invoice"><input type="checkbox" checked={form.invoice_requested} onChange={e => set("invoice_requested", e.target.checked)} /> <span>Solicitar factura <small>(opcional)</small></span></label>{form.invoice_requested && <div className="invoice-fields"><label>RUC*<input required value={form.ruc} onChange={e => set("ruc", e.target.value)} placeholder="Ej. 80012345-6" /></label><label>Razón social*<input required value={form.business_name} onChange={e => set("business_name", e.target.value)} placeholder="Nombre o empresa para la factura" /></label></div>}{msg && <p className="checkout-error">{msg}</p>}<button type="button" className="btn checkout-continue" onClick={() => { if (validateStep1()) goToStep(2); }}>CONTINUAR</button></div></div>}
