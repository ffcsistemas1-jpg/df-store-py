...
export default function Checkout() {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      ...
      if (!session) return;
      const s = createClient();
      s.rpc("save_checkout_draft", { p_session: session, p_draft: { full_name: form.full_name || null, whatsapp: form.whatsapp || null, email: form.email || null, department: form.department || null, city: form.city || null, neighborhood: form.neighborhood || null, address: form.address || null, delivery_type: form.delivery_type || null, payment_method: form.payment_method || null, shipping_company_id: form.shipping_company_id || null, shipping_company_other: form.shipping_company_other || null, preferred_time: form.preferred_time || null, invoice_requested: form.invoice_requested, maps_url: form.maps_url || null, note: form.note || null } }).then(({ error }: any) => { if (error) console.error("No se pudo guardar el borrador", error); });
    }, 700);
    ...
    try {
      ...
      const { data, error } = await s.rpc("create_order", {
        p_customer: {
          ...
          department: form.department || null, city: form.city || null, neighborhood: form.neighborhood || null, address: form.address.trim() || null,
          preferred_time: form.preferred_time || null, invoice_requested: form.invoice_requested, maps_url: form.maps_url.trim() || null,
          note: [form.note.trim(), form.shipping_company_other.trim() ? `Transportadora solicitada por el cliente: ${form.shipping_company_other.trim()}` : ""].filter(Boolean).join("\n") || null
        },
  ...
  return <section className="checkout-section"><div className="checkout-shell"><div className="checkout-brand"><b>DF</b> STORE PY</div><div className="checkout-head"><small>CHECKOUT · PASO {step} DE 3</small><h1>Finalizar compra</h1><p>Completá tus datos. Es rápido y seguro.</p>{restoredDraft && <div className="draft-notice">Retomamos los datos que ya habías completado.</div>}</div><div className="checkout-steps" aria-label="Progreso"><span className={step >= 1 ? "active" : ""}>1. Entrega</span><span className={step >= 2 ? "active" : ""}>2. Revisar pedido</span><span className={step >= 3 ? "active" : ""}>3. Pago</span></div>
    {step === 1 && <div className="checkout-panel panel"><h2>📍 ¿Dónde entregamos?</h2><div className="delivery-choice"><label className={form.delivery_type === "delivery" ? "choice active" : "choice"}><input type="radio" name="delivery" checked={form.delivery_type === "delivery"} onChange={() => set("delivery_type", "delivery")} /><span><b>Asunción y Central</b><small>Delivery y pago al recibir</small></span></label><label className={form.delivery_type === "interior" ? "choice active" : "choice"}><input type="radio" name="delivery" checked={form.delivery_type === "interior"} onChange={() => set("delivery_type", "interior")} /><span><b>Interior del país</b><small>Envío por transportadora</small></span></label></div><div className="delivery-info"><b>🚚 {form.delivery_type === "delivery" ? "Delivery local" : "Envío al interior"}</b><span>{form.delivery_type === "delivery" ? "Pagás al recibir en zonas habilitadas." : "Enviamos por transportadora y podés pagar por transferencia bancaria o Giro Tigo."}</span></div><div className="checkout-fields"><label>Nombre y apellido*<input required value={form.full_name} onChange={e => set("full_name", e.target.value)} placeholder="Tu nombre completo" /></label><label>WhatsApp*<input required value={form.whatsapp} onChange={e => set("whatsapp", e.target.value)} placeholder="09xx xxx xxx" /></label><label>Email <span className="field-optional">(opcional)</span><input type="email" value={form.email} onChange={e => set("email", e.target.value)} placeholder="tu@email.com" /></label>{form.delivery_type === "delivery" && <><div className="location-box"><div><b>📍 Ubicación para facilitar la entrega</b><span>{form.maps_url ? "Ya compartiste tu ubicación." : "Podés compartir tu ubicación exacta o pegar un enlace de Google Maps."}</span>{locationStatus && <small>{locationStatus}</small>}</div><div className="location-actions"><button type="button" className="location-btn" onClick={location}>Usar mi ubicación</button><button type="button" className="location-btn secondary" onClick={pasteMaps}>Pegar enlace</button></div></div><label>Ciudad / Distrito*<select required value={form.city} onChange={e => setDeliveryCity(e.target.value)}><option value="">Seleccioná tu ciudad</option>{deliveryCities.map(c => { const f = deliveryCityFee(c); return <option key={c} value={c}>{c}{f !== null ? ` — ${Number(f).toLocaleString("es-PY")}` : " — Sin tarifa"}</option>; })}</select></label><label>Dirección <span className="field-optional">(opcional si ya compartiste tu ubicación)</span><input value={form.address} onChange={e => set("address", e.target.value)} placeholder="Calle, número y referencia" /></label><label>Barrio / localidad <span className="field-optional">(opcional si ya compartiste tu ubicación)</span>{form.city ? <select value={form.neighborhood} onChange={e => set("neighborhood", e.target.value)} disabled={!neighborhoodsForCity.length}><option value="">{neighborhoodsForCity.length ? "Seleccioná tu barrio / localidad" : "Cargando barrios / localidades..."}</option>{neighborhoodsForCity.map(n => <option key={n} value={n}>{n}</option>)}</select> : <select disabled><option>Primero elegí tu ciudad</option></select>}</label></>}
      ...
      <details className="checkout-more"><summary>+ Más opciones (horario, factura)</summary><label>Horario<select value={form.preferred_time} onChange={e => set("preferred_time", e.target.value)}><option>Mañana</option><option>Tarde</option><option>Cualquier horario</option></select></label><label className="invoice-check"><input type="checkbox" checked={form.invoice_requested} onChange={e => set("invoice_requested", e.target.checked)} /> Necesito factura</label></details><button type="button" className="note-toggle" onClick={() => setShowNote(!showNote)}>+ Agregar una nota</button>{showNote && <label>Nota para el pedido<textarea value={form.note} onChange={e => set("note", e.target.value)} placeholder="Ej. llamar antes de entregar" /></label>}{msg && <p className="checkout-error">{msg}</p>}<button type="button" className="btn checkout-continue" onClick={() => { if (validateStep1()) setStep(2); }}>CONTINUAR</button></div></div>}
