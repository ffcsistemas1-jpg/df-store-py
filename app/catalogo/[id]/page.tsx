...
export default async function Product({params}:{params:Promise<{id:string}>}){
 ...
 return <section><ProductViewTracker id={p.id} name={p.name} price={p.price}/><Link className="back" href="/catalogo">← Volver al catálogo</Link><div className="detail"><div><div className="pic big">{p.image_url?<img src={p.image_url} alt={p.name}/>:<b>DF</b>}</div>{p.video_url&&<video src={p.video_url} controls playsInline className="product-video"/>}</div><div><small>{p.category}</small><h1>{p.name}</h1><div className="price">₲ {Number(p.price).toLocaleString("es-PY")}</div><p>{p.description||"Producto disponible en DF Store PY."}</p><p><b>Stock:</b> {p.stock ?? "Consultar"}</p><AddToCartButton p={p}/></div></div></section>;
}
