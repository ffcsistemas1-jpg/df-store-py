...
function normalizeCategory(value: unknown) {
  ...
  if (category === "Electro") return "Electrónica";
  if (category === "Ropa") return "Vestimenta";
...
export async function PATCH(req: Request) {
  ...
  const payload = {
    name: String(body?.name || "").trim(), price, cost, stock,
    category: normalizeCategory(body?.category),
    description: body?.description ? String(body.description) : null,
    image_url: body?.image_url ? String(body.image_url) : null,
    video_url: body?.video_url ? String(body.video_url) : null,
    active: Boolean(body?.active), updated_at: new Date().toISOString(),
  };
