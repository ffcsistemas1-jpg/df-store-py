...
export async function GET(request: Request) {
  ...
  if (since && until) url.searchParams.set("time_range", JSON.stringify({ since, until }));
  else url.searchParams.set("date_preset", "last_7d");
  ...
  try {
    ...
    return NextResponse.json({
      configured: true, connected: true, period: since && until ? `${since}:${until}` : "last_7d",
      spend, impressions: Number(row.impressions || 0), clicks: Number(row.clicks || 0), purchases,
      purchaseValue, costPerPurchase: purchases > 0 ? spend / purchases : 0,
