...
export function ProductManager({initialProducts}:{initialProducts:Product[]}){
 ...
 async function remove(p:Product){if(!confirm(`¿Eliminar "${p.name}"? Esta acción no se puede deshacer.`))return;setBusy(true);setMsg("");try{const s=createClient();const {data,error}=await s.from("products").delete().eq("id",p.id).select("id");if(error)throw error;if(!data||data.length===0){setMsg("❌ No se pudo eliminar: tu sesión de administrador puede haber vencido. Cerrá sesión, volvé a entrar a /admin/login, e intentá de nuevo.");return}setProducts(x=>x.filter(a=>a.id!==p.id));if(editing?.id===p.id)setEditing(null);setMsg("✅ Producto eliminado.");}catch(e:any){const friendly=e?.code==="23503"?"❌ Este producto ya tiene pedidos asociados. Corré el patch supabase/PATCH-BORRAR-PRODUCTOS-CON-PEDIDOS.sql en Supabase para poder borrarlo igual.":"❌ "+(e?.message||"No se pudo eliminar");setMsg(friendly)}finally{setBusy(false)}}
