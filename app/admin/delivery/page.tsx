...
const formatFeeInput=(value:string)=>{const digits=value.replace(/\D/g,"");return digits?Number(digits).toLocaleString("es-PY"):""};
const parseFeeInput=(value:string)=>Number(value.replace(/\./g,""));
...
export default function Delivery(){
 ...
 function startEdit(c:CoverageCity){const z=cityZone(c.city);setEditingCity(c.city);setFee(z?formatFeeInput(String(z.fee)):"");setMsg("")}
 function cancelEdit(){setEditingCity(null);setFee("")}
 async function saveCity(c:CoverageCity){
   const value=parseFeeInput(fee); if(!Number.isFinite(value)||value<0){setMsg("❌ Ingresá una tarifa válida.");return}
 ...
 async function addZone(e:React.FormEvent){e.preventDefault();setMsg("");const value=parseFeeInput(form.fee);if(!form.city.trim()||!Number.isFinite(value)||value<0){setMsg("❌ Ciudad y costo válido son obligatorios.");return}const payload={department:form.department.trim(),city:form.city.trim()||null,neighborhood:form.neighborhood.trim()||null,fee:value,active:form.active};const r=await s.from("delivery_zones").insert(payload);if(r.error)setMsg("❌ "+r.error.message);else{setMsg("✅ Zona agregada.");resetForm();setShowAdd(false);load()}}
 ...
 {msg&&<p>{msg}</p>}
 {loading?<p>Cargando...</p>:<div className="delivery-city-list">{COVERAGE.map(c=>{const z=cityZone(c.city);const editing=editingCity===c.city;return <div className="delivery-city-row" key={c.city}><div><strong>{c.city}</strong><small>{z?.active===false?"Inactiva":"Delivery disponible"}</small></div>{editing?<div className="delivery-fee-edit"><span>₲</span><input autoFocus type="text" inputMode="numeric" value={fee} onChange={e=>setFee(formatFeeInput(e.target.value))} placeholder="20.000"/><button className="btn" disabled={saving} onClick={()=>saveCity(c)}>{saving?"Guardando...":"Guardar"}</button><button className="linkbtn" onClick={cancelEdit}>Cancelar</button></div>:<div className="delivery-fee"><strong>{z?money(z.fee):"Sin tarifa"}</strong><button className="edit-pencil" aria-label={`Editar tarifa de ${c.city}`} onClick={()=>startEdit(c)}>✏️</button></div>}</div>})}</div>}
 </div>
