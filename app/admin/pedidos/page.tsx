...
export default function Pedidos(){
 ...
 return <section><div className="title"><div><small>ADMINISTRADOR</small><h1>Pedidos</h1></div><Link href="/admin">← Admin</Link></div>{msg&&<div className="panel">⚠️ {msg}</div>}{loading?<div className="panel">Cargando pedidos...</div>:!orders.length?<div className="empty"><h2>Aún no hay pedidos</h2><p>Los pedidos confirmados desde el checkout aparecerán aquí.</p></div>:<div className="admin-orders">{orders.map(o=><article className="panel order-card" key={o.id}><div className="order-head"><div><small>#{o.id.slice(0,8).toUpperCase()}</small><h2>{o.customer?.full_name||"Cliente"}</h2><p>{o.customer?.whatsapp||""} · {o.customer?.city||""}, {o.customer?.department||""}</p></div><strong>{money(o.total)}</strong></div><div className="order-meta"><span>📦 {o.delivery_type}</span><span>💳 {o.payment_method}</span><span>🚚 {o.shipping_company?.name||"—"}</span><span>{new Date(o.created_at).toLocaleString("es-PY")}</span></div><div className="order-actions"><label>Estado<select value={o.status} onChange={e=>change(o.id,e.target.value)}>{statuses.map(s=><option key={s} value={s}>{s}</option>)}<option value="__prueba__">🧪 Prueba (eliminar)</option></select></label><Link className="btn secondary" href={`/admin/pedidos/${o.id}`}>Ver detalle</Link></div></article>)}</div>}</section>;
}
